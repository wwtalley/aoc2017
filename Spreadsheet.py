...
for row in SHEET:
    ...
    x = 0
    for x in range(0, len(sorted_row)):
        for y in range(x+1, len(sorted_row)):
            print("Pair=", sorted_row[x], sorted_row[y])
            if(isDivisible(sorted_row[x], sorted_row[y])):
                matches += 1
                print("YES:", sorted_row[x], sorted_row[y])
                checksum += sorted_row[x] / sorted_row[y]

    print("Row", row, "Complete" )

print("Answer is:", checksum)
